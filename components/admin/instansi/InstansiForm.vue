<template>
  ...
  >
    ...
    <div class="flex items-center justify-between">
      ...
      >
        {{ isEditing ? "Perbarui" : "Tambah" }} Instansi
      </button>
      <NuxtLink
        to="/admin/instansi"
      ...
      try {
        ...
        if (this.isEditing) {
          response = await this.$axios.$put(
            `/instansi/${this.instansi.id}`,
            this.form
          );
        } else {
          response = await this.$axios.$post("/instansi", this.form);
        }
        ...
        this.formError =
          ...
          e.message ||
          "Terjadi kesalahan saat menyimpan instansi.";
        if (e.response?.data?.errors) {
          const errors = Object.values(e.response.data.errors).flat();
          this.formError = errors.join(", ");
        }
        console.error("Error submitting instansi form:", e);
      }
    },
  },
};
