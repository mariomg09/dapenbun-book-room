    ...
    <table class="min-w-full divide-y divide-gray-200">
      ...
      <tbody class="bg-white divide-y divide-gray-200">
        <tr v-for="booking in bookings" :key="booking.id">
          ...
          <td class="px-6 py-4 whitespace-nowrap">{{ booking.room ? booking.room.name : 'N/A' }}</td>
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <span :class="statusBadgeClass(booking.status ? booking.status.name : '')">
...
export default {
  ...
  computed: {
    ...
    pendingStatusId() { return 1; },
    pimpinanApprovedStatusId() { return 2; },
    approvedStatusId() { return 3; },
    rejectedStatusId() { return 4; },
  },
  methods: {
    ...
    statusBadgeClass(statusName) {
      switch (statusName) {
        ...
        case 'pimpinan_approved': return 'bg-blue-100 text-blue-800';
    ...
    canEditOrDelete(booking) {
      ...
      const isNotFinal = booking.status.name !== 'approved' && booking.status.name !== 'rejected';
    ...
    canApproveRejectButton(booking) {
      ...
      if (bookingStatusName === 'approved' || bookingStatusName === 'rejected') {
        return false;
      }
      ...
      if (isCombinedHRandPimpinan) {
          ...
          const bookingUserDeptId = booking.user ? booking.user.department_id : null;

          // Jika departemen sama: bisa pending atau pimpinan_approved
          if (currentUserDeptId === bookingUserDeptId) {
              return bookingStatusName === 'pending' || bookingStatusName === 'pimpinan_approved';
          }
      ...
      if (isPurePimpinan) {
          ...
          const currentUserDeptId = this.user ? this.user.department_id : null;
          const bookingUserDeptId = booking.user ? booking.user.department_id : null;
          return bookingStatusName === 'pending' && currentUserDeptId === bookingUserDeptId;
      }

      // HR Murni: Policy kita melarang HR murni untuk approve/reject
      if (isHR && !isPurePimpinan) { // hasRole('HR') tapi bukan pimpinan murni
          return false;
      }
    ...
    async deleteBooking(id) {
      ...
      try {
        ...
        alert('Gagal menghapus booking: ' + (e.response?.data?.message || e.message));
    ...
    async approveBooking(id) {
      ...
      try {
        ...
        alert('Gagal menyetujui booking: ' + (e.response?.data?.message || e.message));
    ...
    async rejectBooking(id) {
      ...
      try {
        ...
        alert('Gagal menolak booking: ' + (e.response?.data?.message || e.message));
